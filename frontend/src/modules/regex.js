...
const regex = /^(?<startDay>(?:(?:mon|fri|sun)(?:day)?|(?:tue(?:s)?(?:day)?|wed(?:nesday)?|thur(?:s)?(?:sday)?|sat(?:urday)?))\b)\s*(?<startHour>(?:[1-9]|[1][0-2]))(?::(?<startMinute>(?:[0-5][0-9])))?\s*(?:(?<startHalfday>[ap])[m]?)?\s*-\s*(?<endDay>(?:(?:mon|fri|sun)(?:day)?|(?:tue(?:s)?(?:day)?|wed(?:nesday)?|thur(?:s)?(?:sday)?|sat(?:urday)?))\b)?\s*(?<endHour>[0-9]{1,2})(?::(?<endMinute>(?:[0-5][0-9])))?\s*(?:(?<endHalfday>[ap])[m]?)?$/;
...
const parseEvents = (data, tz = "local") => {
    let parsedEvents = data.split(",").map((item) => {
        ...
        if (
            regexResult.groups.startHalfday &&
        ...
        if (regexResult.groups.endHalfday) {
            if (regexResult.groups.endHalfday.toLowerCase() === "p") {
                endHour += 12;
            }
        } else {
            if (
