...
import { CalendarEventModel } from "models/out/main";
...
const parseDaysUntilNext = (startDay: string, tz: string) => {
...
const parseEvents = (data: string, tz: string = "local") => {
    let parsedEvents = data.split(",").map((item) => {
        ...
        let event: CalendarEventModel = CalendarEventModel.fromDateTime(
            startTime,
            endTime
        );
        return event;
    ...
    let filteredEvents = parsedEvents.filter(
        (item): item is CalendarEventModel => item !== undefined
    );
    ...
    if (filteredEvents.length > 1) {
        for (let i = 1; i < filteredEvents.length; i++) {
            ...
            if (
                ...
                DateTime.fromISO(filteredEvents[i - 1]!.start)
            ) {
                e.start = DateTime.fromISO(e.start).plus({ days: 7 }).toISO();
                e.end = DateTime.fromISO(e.end).plus({ days: 7 }).toISO();
            }
        }
    }
