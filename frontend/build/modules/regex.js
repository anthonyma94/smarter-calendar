...
const main_1 = require("../../../../@types/common/main");
...
const parseDaysUntilNext = (startDay, tz) => {
    let d = luxon_1.DateTime.local().setZone(tz);
...
const parseEvents = (data, tz = "local") => {
    let parsedEvents = data.split(",").map((item) => {
        ...
        if (regexResult.groups.startHalfday) {
            if (regexResult.groups.startHalfday.toLowerCase() === "a" &&
                startHour === 12)
                startHour -= 12;
            else if (regexResult.groups.startHalfday.toLowerCase() === "p" &&
                startHour !== 12)
                startHour += 12;
        }
        else {
        ...
        if (regexResult.groups.endHalfday) {
            if (regexResult.groups.endHalfday.toLowerCase() === "a" &&
                endHour === 12)
                endHour -= 12;
            else if (regexResult.groups.endHalfday.toLowerCase() === "p" &&
                endHour !== 12)
                endHour += 12;
        }
        else {
        ...
        // Using Luxon
        let startTime = luxon_1.DateTime.fromFormat(`${date} ${startHour}:${startMinute}`, format, { zone: tz });
        let endTime = luxon_1.DateTime.fromFormat(`${endDate || date} ${endHour}:${endMinute}`, format, { zone: tz });
        ...
        let event = main_1.CalendarEventModel.fromDateTime(startTime, endTime);
        return event;
    });
    let filteredEvents = parsedEvents.filter((item) => item !== undefined);
    // Check that events come after the previous event
    if (filteredEvents.length > 1) {
        for (let i = 1; i < filteredEvents.length; i++) {
            let e = filteredEvents[i];
            if (luxon_1.DateTime.fromISO(e.start) <
                luxon_1.DateTime.fromISO(filteredEvents[i - 1].start)) {
                e.start = luxon_1.DateTime.fromISO(e.start).plus({ days: 7 }).toISO();
                e.end = luxon_1.DateTime.fromISO(e.end).plus({ days: 7 }).toISO();
            }
        }
    }
